...
function imageFor(name: string): string {
  return `/actors/${name.replace(/ /g, "_").toLowerCase()}.jpg`;
}
...
export function generateActorQuestions(_level: number, seed: string): ImageMCQuestion[] {
  const rng = mulberry32(`${seed}::actors`);
  const pool = shuffleInPlace([...ACTORS], rng);
  const picked = pool.slice(0, Math.min(ROUND_QUESTIONS, pool.length));

  return picked.map((actor) => {
    const decoyPool = shuffleInPlace([...actor.decoys], rng).slice(0, 3);
    const options = shuffleInPlace([actor.name, ...decoyPool], rng);
    const answerIndex = options.indexOf(actor.name);
    return {
      imageSrc: imageFor(actor.name),
      imageAlt: "Who is this actor?",
      caption: "Who is this actor?",
      options,
      answerIndex,
    };
  });
